import { StyleSheet, SafeAreaView, View, StatusBar, TextInput } from "react-native";
...
export default function HomeScreenCustomer({ navigation, allFoodItems, userName, refreshData, userType, foodItemAction }) {
  ...
  const handleSearchQueryChange = (query) => {
    ...
    const filteredItems = allFoodItems.filter((data) => {
      return data.foodName.toLowerCase().includes(query.toLowerCase()) || 
  ...
  return search ? (
    <SafeAreaView style={styles.container}>
      ...
      <TextInput
        ...
        placeholder="Search for food by location or name..."
        value={searchQuery}
      ...
      <View style={styles.medicationSection}>
        {filteredFoodItems && 
          <FlatList
            data={filteredFoodItems.filter((data) => data.stillAvailable === true)}
            renderItem={(data) => (
              <FoodItem
                title={data.Name}
                props={data}
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.medicationSection}>
        {allFoodItems && 
          <FlatList
            data={allFoodItems.filter((data) => data.stillAvailable === true)}
            renderItem={(data) => (
              <FoodItem
                title={data.Name}
                props={data}
